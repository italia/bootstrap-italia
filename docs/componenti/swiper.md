...
Il carousel è una presentazione per scorrere una serie di contenuti, costruito con trasformazioni CSS 3D e un po' di JavaScript. Funziona con una serie d'immagini, testo o codice personalizzato. Include il supporto per i controlli, gli indicatori precedente/successivo e i dots che indicano la posizione corrente.

Il plugin utilizzato è [Swiper](https://swiperjs.com/).
Swiper è il più moderno slider touch gratuito che utilizza l'accellazione hardware per le transizioni. È progettato per l'uso in siti web responsive, app web, app native e ibride. Swiper è il componente slider predefinito in Ionic Framework.
